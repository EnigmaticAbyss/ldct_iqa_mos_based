\begin{tikzpicture}[
  ...
  \node[model] (sft-trl) at (5.36,2.76) {TRL \texttt{SFTTrainer}\\assistant-only loss};
  \node[model] (sft-model) at (5.36,1.78) {MedGemma 1.5\\+ LoRA adapter};
  \node[loss] (sft-out) at (5.36,0.70) {Generated target\\\texttt{\{"rating": y\}}};
  ...
  \node[model] (grpo-gen) at (9.16,1.78) {MedGemma 1.5 + LoRA\\group of 4 completions};
  \node[loss] (grpo-reward) at (9.16,0.70) {MOS reward\\$R(\hat{y},y)=-|\hat{y}-y|$};
