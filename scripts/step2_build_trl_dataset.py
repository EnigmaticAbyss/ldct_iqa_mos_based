...
def build_records(images_dir: Path,
                  mos_json: Path,
                  system_prompt: str,
                  assistant_template: str,
    ...
    for i, stem in enumerate(tqdm(common, desc=f"Building records from {images_dir.name}", leave=True)):
        mos = mos_map[stem]
...
def save_hf_dataset(rows: List[Dict[str, Any]], out_dir: Path, logger: logging.Logger):
    if not HF_AVAILABLE:
...
# --------------------- Grouping / Splitting ----------------------
@dataclass
class TmpGrouping:  # minimal runtime container
    mode: str
    regex: str = "^(.*?)[_.-]\\d+$"
    prefix_len: int = 0
...
def make_group_id(stem: str, grouping: TmpGrouping) -> str:
    ...
    if grouping.mode == "none":
        return s                    # each file is its own group
...
def main():
    ap = argparse.ArgumentParser(description="Step 2: Build TRL-ready datasets from PNGs + MOS, with optional slice/group-wise train/val split.")
    ...
    for subset in cfg.subsets:
        ...
        if cfg.output_hf_dataset:
            save_hf_dataset(rows, cfg.hf_out_root / f"{subset.name}_dataset", logger)
    ...
    if cfg.val_split is not None:
        ...
        if pool_name not in built_rows:
            ...
            if cfg.output_hf_dataset:
                save_hf_dataset(train_rows, cfg.hf_out_root / "train_dataset", logger)
                save_hf_dataset(val_rows,   cfg.hf_out_root / "val_dataset",   logger)

            # stats entries
            def _count(rows: List[Dict[str, Any]]):
                cc: Dict[str, int] = {}
                for r in rows: cc[r["metadata"]["category"]] = cc.get(r["metadata"]["category"], 0) + 1
                return cc
